...
func TestCacheBinLog(t *testing.T) {
	tc, _ := New(DefaultExpiration, 0, "binlog.test")
...
func TestCacheTimes(t *testing.T) {
	...
	tc, _ := New(50*time.Millisecond, 1*time.Millisecond, "")
	tc.Set("a", 1, DefaultExpiration)
...
func TestNewFrom(t *testing.T) {
	...
	tc, _ := NewFrom(DefaultExpiration, 0, "", m)
	a, found := tc.Get("a")
	if !found {
...
func TestExpired(t *testing.T) {
	...
	<-time.After(25 * time.Millisecond)
	expired = tc.items["c"].Expired()
	if !expired {
		t.Error("Found c when it should have been automatically deleted")
	}

	<-time.After(30 * time.Millisecond)
	expired = tc.items["a"].Expired()
	if !expired {
		t.Error("Found a when it should have been automatically deleted")
	}

	expired = tc.items["b"].Expired()
	if expired {
		t.Error("Did not find b even though it was set to never expire")
	}

	expired = tc.items["d"].Expired()
	if expired {
		t.Error("Did not find d even though it was set to expire later than the default")
	}
...
func benchmarkCacheSetBinLog(b *testing.B, exp time.Duration) {
	b.StopTimer()
	tc, _ := New(exp, 0, "binlog.test")
	b.StartTimer()
	for i := 0; i < b.N; i++ {
		tc.Set("foo", "bar", DefaultExpiration)
	}
...
func BenchmarkCacheSetDeleteBinLog(b *testing.B) {
	b.StopTimer()
	tc, _ := New(DefaultExpiration, 0, "binlog.test")
	b.StartTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkCacheSetDeleteSingleLockBinLog(b *testing.B) {
	b.StopTimer()
	tc, _ := New(DefaultExpiration, 0, "binlog.test")
	b.StartTimer()
	for i := 0; i < b.N; i++ {
...
func BenchmarkDeleteExpiredLoopBinLog(b *testing.B) {
	b.StopTimer()
	tc, _ := New(5*time.Minute, 0, "binlog.test")
	tc.mu.Lock()
	for i := 0; i < 100000; i++ {
		tc.set(strconv.Itoa(i), "bar", DefaultExpiration)
	}
